...
void initPlayer() {
    ...
    player.wallJumpSpeed = player.jumpSpeed;
...
void handlePlayerInput() {
    ...
    if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0))))) {
        if (touchingGround()) {
            ...
            player.isIdle = 1;
            player.curFrame = 0;
        } else if (player.canWallJump && (collisionLeft(2) || collisionRight(2))) {
            player.isJumping = 1;
            player.rdel = -player.wallJumpSpeed;
            player.isIdle = 1;
            player.curFrame = 0;
        }
...
void laserSlingAnimation() {
    ...
    curFrame++;
    if (nearestLaser->laser->type > 1) {
        shadowOAM[65 +11 +1].attr0 = ((((nearestLaser->laser->screenRow) >> 4) - signOf(nearestLaser->distance) * 4) & 0xFF)
                                        | (0<<8) | (0<<13) | (0<<14);
        shadowOAM[65 +11 +1].attr1 = (((player.screenCol) >> 4) & 0x1FF)
                                        | (0<<14);
    } else {
        shadowOAM[65 +11 +1].attr0 = (((player.screenRow) >> 4) & 0xFF)
                                        | (0<<8) | (0<<13) | (0<<14);
        shadowOAM[65 +11 +1].attr1 = ((((nearestLaser->laser->screenCol) >> 4) - signOf(nearestLaser->distance) * 4) & 0x1FF)
                                        | (0<<14);
    }
    if (numFrames < 12) {
        shadowOAM[65 +11 +1].attr2 = ((curFrame / (numFrames / 16))*32+(10 +3)) | ((0)<<12);
    } else {
        shadowOAM[65 +11 +1].attr2 = ((max(3 - (curFrame - 12) / (numFrames / 16), 0))*32+(10 +3)) | ((0)<<12);
    }

    if (curFrame > numFrames) {
        curFrame = 0;
        player.specialAnim = 0;
        shadowOAM[65 +11 +1].attr0 = (2<<8);
        finishLaserSling();
