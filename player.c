...
void laserSlingAnimation() {
    ...
    if (numFrames < 12) {
        shadowOAM[NUMLASERS+LASERINDEX+1].attr2 = ATTR2_TILEID(SPRITESHEETINDEX+3, curFrame / (numFrames / 16)) | ATTR2_PALROW(0);
    } else {
        shadowOAM[NUMLASERS+LASERINDEX+1].attr2 = ATTR2_TILEID(SPRITESHEETINDEX+3, max(3 - (curFrame - 12) / (numFrames / 16), 0)) | ATTR2_PALROW(0);
    }
