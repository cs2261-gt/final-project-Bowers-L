...
# 49 "laser.c"
    for (int i = 45; i < 56; i++) {
        initLaser(&lasers[i], 464, 216 + (i-45)*16, 1);
    }

    initLaser(&lasers[57], 416, 144, 1);

    initLaser(&lasers[58], 376, 136, 3);
    initLaser(&lasers[59], 392, 136, 2);

    initLaser(&lasers[60], 344, 136, 3);
    initLaser(&lasers[61], 360, 136, 2);

    initLaser(&lasers[62], 336, 112, 1);

    initLaser(&lasers[63], 296, 104, 3);
    initLaser(&lasers[64], 312, 104, 2);
    ...
    for (int i = 0; i < 65; i++) {
        if (lasers[i].active && !lasers[i].hide) {
            ...
            if (lasers[i].type > 1) {
                ...
                perpDistance = player.worldCol - lasers[i].worldCol;
                length = lasers[i].width;
            } else {
                distance = lasers[i].worldCol - player.worldCol;
                perpDistance = player.worldRow - lasers[i].worldRow;
                length = lasers[i].height;
            }

            if (distance < ((16) << 4) && distance > -((16) << 4)
                && perpDistance >= 0 && perpDistance < length) {
