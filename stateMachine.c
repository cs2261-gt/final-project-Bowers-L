...
void initSplash() {
    ...
    REG_BG0CNT = BG_4BPP | BG_SIZE_SMALL | BG_CHARBLOCK(0) | BG_SCREENBLOCK(22);
    DMANow(3, SplashScreen_StartTiles, &CHARBLOCK[0], SplashScreen_StartTilesLen/2);
    DMANow(3, SplashScreen_StartMap, &SCREENBLOCK[22], SplashScreen_StartMapLen/2);
...
void initInstructions() {
    ...
    DMANow(3, InstructionsScreenMap, &SCREENBLOCK[22], InstructionsScreenMapLen/2);
}
...
void initPause() {
    ...
    DMANow(3, PauseScreen_ResumeMap, &SCREENBLOCK[22], PauseScreen_ResumeMapLen/2);
    DMANow(3, PauseScreen_ResumePal, PALETTE, 256);
...
void initWin() {
    ...
    DMANow(3, WinScreenMap, &SCREENBLOCK[22], WinScreenMapLen/2);
...
void updateSplash() {
    if (BUTTON_PRESSED(BUTTON_DOWN) && (menuState == OPTSTART)) {
        ...
        DMANow(3, SplashScreen_InstructionsMap, &SCREENBLOCK[22], SplashScreen_InstructionsMapLen/2);
    }
...
void updatePause() {
    if (BUTTON_PRESSED(BUTTON_DOWN) && (menuState == OPTRESUME)) {
        ...
        DMANow(3, PauseScreen_QuitMap, &SCREENBLOCK[22], PauseScreen_QuitMapLen/2);
    }
