...
void playSoundA(const signed char* sound, int length, int loops) {
    dma[1].cnt = 0;
    DMANow(1, sound, REG_FIFO_A, DMA_DESTINATION_FIXED | DMA_REPEAT | DMA_AT_REFRESH | DMA_32);

    REG_TM0CNT = 0;
    REG_TM0D = -(PROCESSOR_CYCLES_PER_SECOND / SOUND_FREQ);
    REG_TM0CNT = TIMER_ON;
...
void playSoundB(const signed char* sound, int length, int loops) {
    dma[2].cnt = 0;
    DMANow(2, sound, REG_FIFO_B, DMA_DESTINATION_FIXED | DMA_REPEAT | DMA_AT_REFRESH | DMA_32);

    REG_TM1CNT = 0;
    REG_TM1D = -(PROCESSOR_CYCLES_PER_SECOND / SOUND_FREQ);
    REG_TM1CNT = TIMER_ON;
